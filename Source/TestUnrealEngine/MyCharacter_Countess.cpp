...
void AMyCharacter_Countess::BeginPlay()
{
	...
	{
		DefaultScriptActor->OnKillDusk.AddUObject(this, &AMyCharacter_Countess::CatchDusk);
		DefaultScriptActor->OnKillDown.AddUObject(this, &AMyCharacter_Countess::CatchDown);
		DefaultScriptActor->OnKillBuff.AddUObject(this, &AMyCharacter_Countess::CatchWhite);
	}
}
...
void AMyCharacter_Countess::RefreshUI()
{
	...
	if (GameMode)
	{
		...
		if (MyHUD)
		{
			const FString LevelStr = FString::Printf(TEXT("Level : %01d"), CountessLevel);
			MyHUD->LevelText->SetText(FText::FromString(LevelStr));
		}
	}
}

void AMyCharacter_Countess::CatchDusk()
{
	TArray<AActor*> Gates;
...
void AMyCharacter_Countess::CatchDown()
{
	TArray<AActor*> Gates;
...
void AMyCharacter_Countess::CatchWhite()
{
	TArray<AActor*> Gates;
...
void AMyCharacter_Countess::UseAttackEffect()
{
	...
	if (World)
	{
		if (AttackEffect)
		{
			FTransform a(FRotator::ZeroRotator, FVector(-7.f, 3.f, 0.f));
		...
		if (AttackSound)
		{
			UGameplayStatics::SpawnSoundAtLocation(World, AttackSound, GetActorLocation());
		}
	}
}
